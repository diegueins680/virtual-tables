...
class YiiBase
{
	...
	public static function autoload($className)
	{
		...
		{
			if($className == 'DbConnectionConfig')
			{
				//var_dump(dirname(dirname(YII_PATH)).'/_inc/classes/' . $className . '.php');
				//var_dump($_SERVER['DOCUMENT_ROOT'] . '_inc/classes/' . $className . '.php');
				print_r(self::$_coreClasses);
				die;
				include(dirname(dirname(YII_PATH)).'/_inc/classes/DBConnectionConfig.php');
				//var_dump($className);
				//var_dump($className == 'DBConnectionConfig');
				//die;
			}
			...
			if(strpos($className,'\\')===false)  // class without namespace
			{
				...
				elseif(file_exists( $_SERVER['DOCUMENT_ROOT'] . '_inc/classes/' . $className . '.php') )
				{
					include($_SERVER['DOCUMENT_ROOT'] . '_inc/classes/' . $className . '.php');
				}
				else
		...
		if(isset($params[0])) // number choice
		{
			if(strpos($message,'|')!==false)
			{
				if(strpos($message,'#')===false)
				{
					...
					if(($n=min(count($chunks),count($expressions))) !== false)
					{
						for($i=0;$i<$n;$i++)

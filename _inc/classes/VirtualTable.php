 ...
 *
 */
class VirtualTable extends DataBaseAccessor
{
	...
	public function getDbPrimaryKey()
	{
		if(!isset($this->dbPrimaryKey))
		{
			if($this->exists())
			{
				...
				$PKcolumn = DBColumn::getInstanceBy(array(array('column_id', $columnTable->column_id, '=')));
	...
	public function setColumnsFromResults($params)
	{
		...
		$columns = array();
		$pKeyColumn = VirtualColumn::getInstance();
		...
		{
			$column = VirtualColumn::getInstance();
			...
			if(in_array($column->type, VirtualColumn::$typesWithLength))
			{

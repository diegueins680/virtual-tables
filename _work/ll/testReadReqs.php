...
function getConcatenatedSegment(&$params)
{
	...
	if(isset($params['leadArray']))
	{
		...
		{
			$concatenatedSegment .= getFieldToImport($segmentBy, $params).'_'; 
		}
	...
	$concatenatedSegment = trim($concatenatedSegment, '_');
...
//print(var_export($params['requirements'], true));
//die;
//require('myArray.php');
//$params['requirements'] = $myArray;
function getReqForSegment(&$params, $segment=null)
{
	if(empty($segment))
	{
...
function getLeadToImport(&$params)
{
	...
	foreach($params['headersForImport']['fields'] as $key => $importHeader)
	{
		...
		//xdebug_break();
		if(isset($params['headersForImport'][$key]['outputPosition']))
		{
			$outputPosition = $params['headersForImport'][$key]['outputPosition'];
...
function getFieldFormats(&$params)
{
	...
	{
		$phone = strtr($params['leadArray'][$params['position']], array('(' => '', ')' => '', '-' => '', ' ' => ''));
			...
			[
				...
				[
					...
					[
						...
						'function' => 	function(&$params) use($concatenate2)
						{
							return $concatenate2(['New Street', ' ', 'New Apt'], $params['leadArray'], $params['headersForImport']['fields']['street']);
						}
					],
					'city' =>
					[
					...
					[
						...
						[
							...
							[
								...
								'function' =>
								function(&$params)use($concatenate2)
								{
									return $concatenate2(['Billing Address 1', ' ', 'Billing Address 2', ' ', 'Billing Address 3'], $params['leadArray'], $params['headersForImport']['fields']['street']);
								}
							],
							'city' =>
							[
						...
						[
							...
							[
								...
								'function' =>
									function() use (&$params)
									{
										$reservedString = [' JCPL_', ' PSEG_', ' PECO_', ' PEPCO', ' COMED'];
										foreach($reservedString as $reservedString)
										{
											if(($reservedStringPosition = strpos($params['leadArray'][$params['position']], $reservedString)) !== false)
											{
												return substr($params['leadArray'][$params['position']], 0, stripos($params['leadArray'][$params['position']], $reservedString));
											}
										}
										return $params['leadArray'][$params['position']];
									}
							],
							'phoneNumber'  =>
							[
